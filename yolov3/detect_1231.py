...
def detect(save_img=True):
    ...
    with torch.no_grad():
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            for i, det in enumerate(pred):  # detections for image i
                ...
                if det is not None and len(det):
                    ...
                    total = []
                    object_names = []
                    # print('[Det] num of det: {}\n'.format(len(det)))
                    # Write results

                    for *xyxy, conf, cls in reversed(det):
                        # label = '%s %.2f' % (names[int(cls)], conf)
                        print('[Label] result: {}(conf: {})\n'.format(cls, conf))
                        answer.append(int(cls))
                        # answer2.append(int(conf))
                        for i in set(answer):
                            if len(set(answer)) == 1 and i == 0:
                                answer.remove(0)
                                answer.append('사진을 다시 등록해주세요')
                            elif i == 0:
                                answer.remove(0)
        ...
        with open('./classificaion_result.txt', 'w') as f:
            rslt = [r + '\n' for r in rslt]
            f.writelines(rslt)
        for j, i in zip(range(len(answer)), answer):
            if str(i) == '사진을 다시 등록해주세요':
                continue
            elif int(i) > 56 and int(i) < 124:
                answer[j] = i - 1
            elif int(i) > 125 and int(i) < 181:
                answer[j] = int(i) - 2
            elif int(i) > 180 and int(i) < 396:
                answer[j] = int(i) - 1
            elif int(i) > 397:
                answer[j] = int(i) - 3
        print(answer)
